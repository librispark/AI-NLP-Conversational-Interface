...
def punctuate_return(predict, word_vocabulary, punctuation_vocabulary, reverse_punctuation_vocabulary, reverse_word_vocabulary, show_unk, text):
    ...
    while True:
        ...
        for j in range(step):
            return_text += " " + punctuations[j] + " " if punctuations[j] != data.SPACE else " "
            if j < step - 1:
                return_text += subsequence[1+j]
...
def initialize_punctuator(model_file, show_unk_vals=False):
    if model_file:
        model_file = model_file
    else:
        sys.exit("Model file path argument missing")

    show_unk = show_unk_vals
    ...
    return (predict, word_vocabulary, punctuation_vocabulary, reverse_punctuation_vocabulary, reverse_word_vocabulary, show_unk)
